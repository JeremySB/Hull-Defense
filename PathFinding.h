...
public:
	...
	std::stack<int*> findPath(Structure* target);
    bool nextStep(Entity* target);
    void discoverAdjacent(int coor[2]);
//    void loadMap(int map[7][6],int x,int y);
	void loadMap(StructureGrid *map);
	Tile generateTile(int y,int x, Structure* target);

private:
//	DoublyLinkedList<Tile> discovered;
	std::priority_queue<Tile, std::vector<Tile*>, ptr_less<Tile*>> discovered;
